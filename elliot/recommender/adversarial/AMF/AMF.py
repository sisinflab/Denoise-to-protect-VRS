...
class AMF(RecMixin, BaseRecommenderModel):
    ...
    def __init__(self, data, config, params, *args, **kwargs):
        ...
        self._eps_iter = self._params.eps_iter
        self._nb_iter = self._params.nb_iter
            ...
            if not (it + 1) % self._validation_rate:
                ...
                if self._eval_perturbations:
                    for full_batch in self._sampler.step(self._data.transactions, self._data.transactions):
                        self._model.build_iterative_perturbation(full_batch, self._eps_iter, self._nb_iter)
                    ...
                    for k in adversarial_iterative_result_dict.keys():
                        worsening_iterative = {
                            key: (adversarial_iterative_result_dict[k]['val_results'][key] - result_dict[k][
                                'val_results'].get(
                ...
                print(f'Epoch {(it)}/{self._epochs} loss {loss / steps:.3f}')
