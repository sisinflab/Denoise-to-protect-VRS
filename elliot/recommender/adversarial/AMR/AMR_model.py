...
class AMR_model(keras.Model):
    def __init__(self, factors=200, factors_d=20,
                 ...
                 num_users=100,
                 num_items=100,
                 batch_size=512, defense_learning_rate=0.001, defenses=['none'],
                 name="AMR",
                 **kwargs):
    ...
    def train_step(self, batch, user_adv_train=False):
        ...
        with tf.GradientTape() as t:
            ...
            if user_adv_train:
                ...
                adv_xu_neg, _, _, _, _, _ = self(inputs=(user, neg, tf.range(self._batch_size, self._batch_size * 2)),
                                                 training=True)
    ...
    def bb_category_attack(self, original_image, eps=32, eps_step=12, steps=20, target_class=0):
        ...
        """
        adv_x = projected_gradient_descent(self._ife, original_image, eps, eps_step, steps, np.inf, y=[target_class],
                                           targeted=True)
    ...
    def bb_rank_attack(self, next_eval_batch, attack_target_item, eps=32, steps=20):
        ...
        for step in range(steps):
            ...
            adv_x = tf.clip_by_value(
                self.original_target_image - tf.clip_by_value(perturbed_image + scaled_perturbation - image, -eps, eps),
                [-103.939, -116.779, -123.68], [255 - 103.939, 255 - 116.779, 255 - 123.68])
    ...
    def wb_sign_attack(self, original_image, eps_step=8, eps=32, steps=5):
        ...
        perturbation = tf.Variable(
            initial_value=zeros_initializer(shape=original_image.numpy().shape, dtype=tf.float32), trainable=False)
        # print(f'\t\t\tImg {item}')
        valid_loss = -np.inf
        ...
        for _ in range(steps):
            ...
            with tf.GradientTape() as t:
                val = tf.reduce_sum(tf.reduce_sum((self.Tu * tf.matmul(
                    self.feature_extractor(self.original_target_image - perturbation, training=False), self.E)),
                                                  axis=1) + \
                                    tf.squeeze(tf.matmul(
                                        self.feature_extractor(self.original_target_image - perturbation,
                                                               training=False), self.Bp)))
            ...
            # Scale perturbation to be the solution for the norm=eps rather than norm=1 problem
            adv_x = tf.clip_by_value(
                self.original_target_image - tf.clip_by_value(perturbation + scaled_perturbation, -eps, eps),
                [-103.939, -116.779, -123.68], [255 - 103.939, 255 - 116.779, 255 - 123.68])
    ...
    def insa_attack(self, original_image, eps=32, steps=5):
        ...
        for _ in range(steps):
            ...
            adv_x = tf.clip_by_value(self.original_target_image - tf.clip_by_value(perturbation + grads, -eps, eps),
                                     [-103.939, -116.779, -123.68], [255 - 103.939, 255 - 116.779, 255 - 123.68])
            perturbation.assign(self.original_target_image - adv_x)
