...
class AMR(RecMixin, BaseRecommenderModel):
    ...
    def test_attack(self, original_recs, test_attacked_images_src_folder, items_id_by_popularity):
        ...
        name_file = self._config.path_output_rec_performance + f"attack-amr-lr{self._lr_train_defendant}-bs{self._batch_train_defendant}-{dt_string}.tsv"
        ...
        with open(name_file, 'w') as f:
            ...
            for def_name in self._defenses:  # [0,1]
                ...
                for test_defense_batch in self._sampler.pipeline_test_defense_no_tf(test_attacked_images_src_folder,
                    ...
                    attacked_scores, original_scores = self._model.score_items(
                    ...
                    for user in recs.keys():
                        pool.apply_async(evaluate_prediction,
                                         args=(
                                         recs[user], user, self._adversarial_top_k, attacked_scores, target_item_id,original_scores,),
                    ...
                    f.write(
                        f'{def_name}\t{target_item_id}\t{items_id_by_popularity.tolist().index(target_item_id) + 1}\t{attack_name}\t{eps_name}\t{step_name}\t{np.mean(original_hr)}\t{np.mean(target_hr)}\t{np.mean(attacked_scores > original_scores)}\t{np.mean(attacked_scores - original_scores)}\n')

                if def_name == 'denoiser':
                    print('***** STORING FULL RESULTS ******')
                    # Store Defended Performance
                    denoised_results = self.evaluator.eval(recs)
                    name_file_recomm = self._config.path_output_rec_performance + f"denoised-amr-overall-performance-lr{self._lr_train_defendant}-bs{self._batch_train_defendant}-{dt_string}.tsv"
...
def evaluate_prediction(rec_list, user, adversarial_top_k, attacked_scores, target_item_id, original_scores):
    t, o = 0, 0
    check_target = 0
    check_original = 0
    for k, (pred_item, pred_score) in enumerate(rec_list):
        if k < adversarial_top_k and not (check_target == 1 and check_original == 1):
            if attacked_scores[user].numpy() >= pred_score and check_target == 0:
                t = 1
                check_target = 1
                # print(f'WIN-{attacked_scores[user].numpy()}\t{original_scores[user].numpy()}\t{pred_score} for {user} on the item {pred_item}')
            if target_item_id == pred_item and check_original == 0:
                # print(f'LOSE-{attacked_scores[user].numpy()}\t{original_scores[user].numpy()}\t{pred_score} for {user} on the item {pred_item}')
                o = 1
                check_original = 1
        else:
            break
